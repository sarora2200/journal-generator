// ============================================
// DATA: All journal content (200+ pages)
// ============================================

const DATA = {
    // 15 Cover Designs (expanded)
    covers: [
    ...
    // 5 Themes per month (60 total - expanded)
    monthlyThemes: {
    ...
    // Extended moods (12 options - users can customize)
    moodOptions: [
    ...
    // 40+ Monthly Pages (included in every month)
    monthlyPages: [
...
function generateJournal() {
    ...
    // --- WEEKLY PLANNERS (52 weeks) ---
    if (state.includeWeeklyPlanner) {
        for (let week = 1; week <= 12; week++) {
            pages.push({
                type: 'weekly-planner',
                week: week,
                month: months[week % 12],
                colors: colors,
                font: font
            });
            pageCount++;
    ...
    // --- MONTHLY PAGES (12 months × multiple pages) ---
    for (let monthIndex = 0; monthIndex < months.length && pageCount < targetPages; monthIndex++) {
        ...
        // 1. Month Title + Calendar Page
        pages.push({
            type: 'month',
            month: month,
            theme: chosenTheme,
            moodDoodle: moodDoodle,
        ...
        // 2. Mood Tracker Page (with doodle and moods)
        pages.push({
            type: 'mood-tracker',
            month: month,
            moodDoodle: moodDoodle,
        ...
        // 3. Monthly Pages (trackers for each month)
        const monthlyTrackers = shuffleArray(DATA.monthlyPages).slice(0, 10);
        pages.push({
            type: 'trackers',
            month: month,
            trackers: monthlyTrackers,
            colors: colors,
            font: font
        });
        pageCount++;
    ...
    // --- EXTRA PAGES to reach target ---
    const pageTypes = ['doodles', 'trackers', 'seasonal', 'mood-tracker', 'goals', 'reflection'];
    while (pageCount < targetPages) {
        ...
        if (extraType === 'doodles') {
            ...
            const trackerSet = shuffleArray(DATA.monthlyPages).slice(0, 12);
            pages.push({
                type: 'trackers',
                month: 'Extra',
                trackers: trackerSet,
...
function renderPage(page, index) {
    switch (page.type) {
        ...
        case 'vision-board':
            return renderVisionBoard(page, index);
        case 'goals':
            return renderGoals(page, index);
        case 'yearly-review':
            return renderYearlyReview(page, index);
        case 'weekly-planner':
            return renderWeeklyPlanner(page, index);
        case 'month':
            return renderMonth(page, index);
        case 'mood-tracker':
            return renderMoodTracker(page, index);
        case 'trackers':
            return renderTrackers(page, index);
        case 'doodles':
            return renderDoodles(page, index);
        case 'month-doodles':
            return renderMonthDoodles(page, index);
        case 'seasonal':
            return renderSeasonal(page, index);
        case 'reflection':
            return renderReflection(page, index);
        default:
            return '';
    }
}
...
function renderVisionBoard(page) {
    ...
    const visions = [
        'Career', 'Health', 'Relationships', 'Personal Growth',
        'Travel', 'Creativity', 'Finance', 'Spirituality'
    ];
    ...
    let visionHtml = visions.map(v => `
        <div style="padding: 15px; background: ${colors.secondary}22; border-radius: 12px; border: 2px dashed ${colors.primary}; text-align: center;">
            <div style="font-size: 2rem; margin-bottom: 5px;">${randomPick(['🎯', '💫', '🌟', '✨', '🎪', '🌈', '🦋', '🌺'])}</div>
    ...
    return `
        <div class="journal-page" style="font-family: '${font}', sans-serif; color: ${colors.text}; background: ${colors.background};">
            <div style="text-align: center; border-bottom: 3px solid ${colors.primary}; padding-bottom: 15px; margin-bottom: 20px;">
                ...
                <p style="font-size: 0.9rem; opacity: 0.6; margin-top: 5px;">Dream big! Visualize your goals for the year ahead</p>
            </div>
...
function renderWeeklyPlanner(page) {
    ...
    let daysHtml = days.map(day => `
        <div style="padding: 8px; border-bottom: 1px solid ${colors.secondary}; display: flex; align-items: center; gap: 10px;">
            <span style="font-weight: 600; min-width: 80px; font-size: 0.8rem; color: ${colors.primary};">${day}</span>
    ...
    return `
        <div class="journal-page" style="font-family: '${font}', sans-serif; color: ${colors.text}; background: ${colors.background};">
            ...
            <div style="margin-top: 15px; display: grid; grid-template-columns: 1fr 1fr; gap: 15px;">
                <div style="padding: 10px; background: ${colors.secondary}11; border-radius: 8px;">
                    <div style="font-weight: 600; font-size: 0.8rem; color: ${colors.primary};">🎯 Weekly Goals</div>
                    <div style="font-size: 0.75rem; opacity: 0.5;">1. _______________</div>
                    <div style="font-size: 0.75rem; opacity: 0.5;">2. _______________</div>
                    <div style="font-size: 0.75rem; opacity: 0.5;">3. _______________</div>
                </div>
                <div style="padding: 10px; background: ${colors.secondary}11; border-radius: 8px;">
                    <div style="font-weight: 600; font-size: 0.8rem; color: ${colors.primary};">💡 Weekly Wins</div>
                    <div style="font-size: 0.75rem; opacity: 0.5;">1. _______________</div>
                    <div style="font-size: 0.75rem; opacity: 0.5;">2. _______________</div>
                    <div style="font-size: 0.75rem; opacity: 0.5;">3. _______________</div>
                </div>
...
function renderCover(page) {
    ...
    return `
        <div class="journal-page cover" style="font-family: '${font}', serif; color: ${colors.text}; border: 3px solid ${colors.primary}; background: ${cover.gradient || colors.background};">
            ...
            <p style="font-size: 0.9rem; margin-top: 30px; opacity: 0.5;">${state.pageCount}+ Pages • Created with ❤️</p>
        ...
        <div class="journal-page" style="font-family: '${font}', sans-serif; color: ${colors.text}; background: ${colors.background};">
            ...
            <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 15px; margin-top: 20px;">
                ...
                <div style="padding: 12px; background: ${colors.secondary}22; border-radius: 8px; border-left: 4px solid ${colors.accent};">
                    ...
                    <div style="font-size: 0.75rem; opacity: 0.5; margin-top: 5px;">"${randomPick(['Progress over perfection', 'One day at a time', 'Stay positive', 'Keep going', 'Be present'])}"</div>
                </div>
            </div>
...
function renderMoodTracker(page) {
    ...
    return `
        <div class="journal-page mood-tracker-page" style="font-family: '${font}', sans-serif; color: ${colors.text}; background: ${colors.background};">
            ...
            <div class="mood-doodle" style="border-color: ${colors.primary}; background: ${colors.secondary}22;">
                ${moodDoodle.emoji}
            </div>
            
            <div class="mood-doodle-text" style="color: ${colors.primary};">
                ${moodDoodle.name} • Mood Tracker
            </div>
            
            <p style="font-size: 0.9rem; opacity: 0.6; margin-bottom: 10px;">How are you feeling today? Circle your mood below:</p>
            ...
            <div style="margin-top: 15px; display: grid; grid-template-columns: 1fr 1fr; gap: 10px; width: 100%;">
                <div style="padding: 8px; background: ${colors.secondary}11; border-radius: 8px; font-size: 0.8rem; text-align: center; opacity: 0.7;">
                    ⭐ Highlight: ______________
                </div>
                <div style="padding: 8px; background: ${colors.secondary}11; border-radius: 8px; font-size: 0.8rem; text-align: center; opacity: 0.7;">
...
function renderTrackers(page) {
    ...
    let itemsHtml = trackers.slice(0, 12).map(t => `
        <div class="tracker-item" style="border-color: ${colors.secondary}; background: ${colors.secondary}11;">
            ...
            <div class="dots">
                ${Array(12).fill(0).map(() => `<span class="dot" style="border-color: ${colors.secondary};"></span>`).join('')}
            </div>
        </div>
    `).join('');
    ...
    return `
        <div class="journal-page" style="font-family: '${font}', sans-serif; color: ${colors.text}; background: ${colors.background};">
            ...
            <div class="tracker-grid" style="display: grid; grid-template-columns: 1fr 1fr; gap: 12px;">
                ${itemsHtml}
            </div>
            <p style="text-align: center; margin-top: 20px; font-size: 0.8rem; opacity: 0.5;">Track your habits daily • Check or color each day</p>
        </div>
    `;
}
...
function renderDoodles(page) {
    ...
    return `
        <div class="journal-page" style="font-family: '${font}', sans-serif; color: ${colors.text}; background: ${colors.background};">
            ...
            <div class="doodle-grid" style="display: grid; grid-template-columns: repeat(4, 1fr); gap: 12px;">
                ${doodleHtml}
            </div>
            <p style="text-align: center; margin-top: 20px; font-size: 0.8rem; opacity: 0.5;">Practice drawing these fun shapes and objects</p>
        </div>
    `;
}

// --- MONTH DOODLES (Enhanced) ---
function renderMonthDoodles(page) {
    ...
    return `
        <div class="journal-page" style="font-family: '${font}', sans-serif; color: ${colors.text}; background: ${colors.background};">
            ...
            <p style="text-align: center; margin-top: 20px; font-size: 0.8rem; opacity: 0.5;">Learn to draw these ${month}-themed items</p>
        </div>
    `;
}
...
function renderSeasonal(page) {
    ...
    let seasonHtml = seasons.map(s => `
        <div class="season-card" style="border: 2px solid ${colors.secondary}; background: ${colors.secondary}11;">
            ...
            <ul class="bucket-list" style="list-style: none; padding: 0; text-align: left; font-size: 0.8rem;">
                ${Array(6).fill(0).map(() => `<li style="padding: 4px 0 4px 24px; border-bottom: 1px solid ${colors.secondary}22;">☐ ${randomPick(['Go outside', 'Try something new', 'Read a book', 'Visit a new place', 'Cook a new recipe', 'Call a friend', 'Take a photo', 'Write a letter', 'Start a project', 'Learn a skill', 'Practice gratitude', 'Meditate', 'Go for a walk', 'Try a new hobby'])}</li>`).join('')}
            </ul>
        </div>
    `).join('');
    ...
    return `
        <div class="journal-page" style="font-family: '${font}', sans-serif; color: ${colors.text}; background: ${colors.background};">
            ...
            <div class="seasonal-grid" style="display: grid; grid-template-columns: 1fr 1fr; gap: 15px;">
                ${seasonHtml}
            </div>
...
async function downloadPDF() {
    ...
    try {
        ...
        pdf.save(`2026_mega_journal_${state.pageCount}pages.pdf`);
    } catch (error) {
